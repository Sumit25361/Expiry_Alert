...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['action']) && $_POST['action'] === 'delete') {
        ...
        $stmt = $conn->prepare("DELETE FROM other_items WHERE id = ? AND email = ?");
        ...
        if ($expiry_date <= $today) {
            ...
            $stmt = $conn->prepare("INSERT INTO other_items (email, item_name, mfg_date, expiry_date) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $email, $item_name, $mfg_date, $expiry_date);
            ...
            if ($stmt->execute()) {
                ...
                $notificationService->sendItemAddedNotification($email, $item_name, 'Other', $expiry_date);
...
// Fetch other items
$email = $db->escape($_SESSION['email']);
$result = $conn->query("SELECT * FROM other_items WHERE email='$email' ORDER BY expiry_date ASC");
?>
...
<head>
    ...
    <title>Other Items - Expiry Alert</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container fade-in">
        <div class="header">
            <h2>Other Items Manager</h2>
            <p>Track any other items with expiration or renewal dates</p>
        </div>
        ...
        <div class="form-section">
            <div class="info-tip">
                <span>💡</span>
                <div>
                    <strong>Versatile Tracking:</strong> Use this section for warranties, subscriptions, memberships, or any other items with expiry dates.
                </div>
            </div>
                ...
                <form method="POST" id="itemForm">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="item_name" id="item_name" placeholder="e.g., Warranty, Subscription, Membership" required>
                        </div>
                        <div class="form-group">
                            <label for="mfg_date">Start Date (Optional)</label>
                            <input type="date" name="mfg_date" id="mfg_date">
                        </div>
                        <div class="form-group">
                            <label for="expiry_date">Expiry/Renewal Date</label>
            ...
            <div class="table-container">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Add your first item to start tracking expiry dates</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <script>
        ...
        // Set maximum date to today for start date
