...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    if (isset($_POST['action']) && $_POST['action'] === 'delete') {
        ...
        $stmt = $conn->prepare("DELETE FROM foods WHERE id = ? AND email = ?");
        $stmt->bind_param("is", $id, $email);
        
        if ($stmt->execute()) {
            $success_message = "Food item deleted successfully!";
        } else {
            $error_message = "Error deleting food item.";
        }
        $stmt->close();
    } else {
        // Handle add
        $food_name = $db->escape($_POST['food_name']);
        ...
        if ($expiry_date <= $today) {
            ...
            $stmt = $conn->prepare("INSERT INTO foods (email, food_name, mfg_date, expiry_date) VALUES (?, ?, ?, ?)");
            $stmt->bind_param("ssss", $email, $food_name, $mfg_date, $expiry_date);
            ...
            if ($stmt->execute()) {
                ...
                $notificationService->sendItemAddedNotification($email, $food_name, 'Food', $expiry_date);

            } else {
                $error_message = "Error adding food item.";
            }
            $stmt->close();
        }
    }
...
// Fetch foods
$email = $db->escape($_SESSION['email']);
$result = $conn->query("SELECT * FROM foods WHERE email='$email' ORDER BY expiry_date ASC");
?>
...
<head>
    ...
    <title>Food Expiry - Expiry Alert</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <div class="container fade-in">
        <div class="header">
            <h2>Food Expiry Manager</h2>
            <p>Track your food items to reduce waste and ensure freshness</p>
        </div>
        ...
        <div class="form-section">
            <div class="food-tips">
                <span>💡</span>
                <div>
                    <strong>Pro Tip:</strong> Check expiry dates regularly to reduce food waste and maintain a healthy diet.
                </div>
            </div>
                ...
                <form method="POST" id="foodForm">
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <input type="text" name="food_name" id="food_name" placeholder="e.g., Milk, Bread, Yogurt" required>
                        </div>
                        <div class="form-group">
        ...
        <div class="table-section">
            <div class="table-header">
                ...
                <input type="text" class="search-box" placeholder="Search food items..." id="searchInput">
            </div>
            ...
            <div class="table-container">
                ...
                <?php else: ?>
                    <div class="empty-state">
                        ...
                        <p>Add your first food item to start tracking expiry dates</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
    ...
    <script>
        ...
        document.getElementById('searchInput').addEventListener('keyup', function() {
            ...
            for (let i = 0; i < rows.length; i++) {
                const foodName = rows[i].getElementsByTagName('td')[0];
                if (foodName) {
                    const textValue = foodName.textContent || foodName.innerText;
